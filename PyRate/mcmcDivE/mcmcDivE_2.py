...
p.add_argument('-o',      type=str,   help='prefix  output file')
...
if modern_diversity >= 0:
    n_multi_A[0] = modern_diversity - x_obs[0]
...
# init mcmc file
div_output_file = "%s/%s_%s_mcmcdiv.log" % (path_infile, input_file, prefix)
...
for iteration in range(args.n):
    ...
    if rr < 0.001:
        ...
        if modern_diversity >= 0:
            n_multi[0]   = modern_diversity - x_obs[0]
        n_est = x_obs + n_multi
