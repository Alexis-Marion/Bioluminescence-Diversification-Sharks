...
# epochs_preservation.txt is the file with epochs that will be tested using PPmodeltest from PyRate
...
while [ ! -f result_boxplot.pdf ]; do
		...
		echo "python ../../PyRate/PyRate.py  $1 -j \$1 -qShift epochs_preservation.txt -mG -PPmodeltest > File\$1.log" > tmp_script.sh
		parallel -j $2 bash tmp_script.sh ::: {1..20}
		bash script.sh $cntr
		Rscript model_drafting.r tab.tsv epochs_preservation.txt
		rm tab.tsv
		rm tmp_script.sh
		sleep 1
done
